...
### `LuaEvent:Connect(callbackName, table?)`
Connects a callback function to the event by its name. The callback will be executed when the event is invoked.

#### Parameters:
- `callbackName` (`string`) — Name of function to be called when the event is triggered.
- `table` (`table?`) — Optional table to use as the context (`self`) when calling the callback.
