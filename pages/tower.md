...
To create new tower you need to fill [tower config](pages/towersconfig.md).  
...
| `IsTowerPlaced` | `bool` | get | Checks is tower placed (is `Tower:TowerPlaced` was called) |
...
> Shared fields, events, and stat methods (`Health`, `GiveEffect`, etc.) — see [`GameEntity`](pages/GameEntity.md). On towers, changing `Health` / `MaxHealth` on the server syncs to room players.
...
### `Tower.SetupSynchronizer(health): void`

Setups MultiplayerSyncronizer. Usefull when you need to set Syncronizer

#### Parameters:

- `health` (`number`) — Value of new HP
...
| `PreUpgradeMenuOpen` | `UpgradeMenuUI` | calls before the upgrade menu opens for this tower. Return `false` to prevent opening. If another tower's menu was open, it will be closed first — and the new one will not open. |
