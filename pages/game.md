...
|-------|------|-------------|
| `GameVersion` | `string` | Current version of the game |
...
|---------------------|---------------------------|-----------------------------------------------------------------------------|
| `OnClassRegistered`  | name (`string`), table (`table`) | Invoked when class was registered with `game.RegisterClass(...)` |
...
### `game.GetClassTable(name): table`

Gets table of registered class
...
- `table` — A table of class.

---

### `game.GetNodeTable(): table`

Gets meta table of `Node`

#### Returns:

- `table` — A table of `Node`.
...
### `game.CreateZombie(zombieId): Zombie`

Creates a zombie from [zombie config](pages/zombies_config.md). Applies config stats, loads `ModelPath`, and plays a random `StartAnimation` if configured.

**Read also** [Zombie](pages/zombie.md)

#### Parameters:

- `zombieId` (`string`) — Key from the zombies config JSON.

#### Returns:

- `Zombie` — A new zombie instance. Set `CurrentRoom` and `CurrentTeam`, then call `SpawnOnPath`.

```lua
local zombie = game.CreateZombie("Runner")
zombie.CurrentRoom = room
zombie.CurrentTeam = team
zombie:SpawnOnPath(path3D)
```

---

### `game.GetZombieConfig(zombieId): table`

Returns parsed config data for a zombie type.
...
- `table` — Config fields (stats, model path, etc.).

---

### `game.CreateUserdata(className): userdata`

Creates a raw Godot object without a Lua table wrapper. Lighter than `CreateNode`; does not participate in the Lua node lifecycle.

#### Parameters:

- `className` (`string`) — Godot class name (e.g. `"AudioStream"`, `"Resource"`).

#### Returns:

- `userdata` — Native Godot object.
